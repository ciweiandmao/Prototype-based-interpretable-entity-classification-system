...
class HetGNN_GPU(nn.Module):
    ...
    def forward(self, graph, inputs):
        ...
        # 第二层卷积
        h = self.conv2(graph, h)
        h = {k: self.layer_norm(v) for k, v in h.items()}

        if 'entity' in h:
            ent_feats = h['entity']
            ent_feats = ent_feats.unsqueeze(1)
            ent_feats, _ = self.gru(ent_feats)
            ent_feats = ent_feats.squeeze(1)
            h['entity'] = self.dropout(ent_feats)
...
class FB15K_XGradNet_GPU(nn.Module):
    def __init__(self, hetero_graph, num_entities, num_relations, feature_dim=128, hidden_dim=128, out_dim=64):
        super().__init__()
        self.num_entities = num_entities
        self.num_relations = num_relations
        ...
        # 实体嵌入 - 使用sparse embedding减少内存使用
        self.entity_emb = nn.Embedding(num_entities, feature_dim, sparse=False)

        # 异构图神经网络
        rel_names = list(set(hetero_graph.etypes))
        self.hetgnn = HetGNN_GPU(feature_dim, hidden_dim, out_dim, rel_names)
    ...
    def _init_weights(self):
        ...
        # 初始化链接预测器
        for module in [self.link_predictor]:
    ...
    def forward(self, node_ids=None, return_subgraph=False):
        ...
        if torch.any(node_ids >= self.num_entities):
            ...
            node_ids = node_ids[valid_mask]
            if len(node_ids) == 0:
                empty_tensor = torch.empty(0, self.out_dim, device=device)
                if return_subgraph:
        ...
        if graph.device != device:
            try:
                ...
                print(f"无法移动图到GPU: {e}，使用CPU")
                device = torch.device('cpu')
                return self._forward_cpu(node_ids, return_subgraph)
        ...
        if return_subgraph:
            # 构建子图嵌入
            subgraph_embeddings = []
            entity_embeddings = node_embeddings['entity']

            for i, entity_id in enumerate(node_ids.tolist()):
                sub_emb = self.subgraph_builder.build_subgraph_embedding(
                    entity_id, entity_embeddings
                )
                subgraph_embeddings.append(sub_emb)

            if subgraph_embeddings:
                subgraph_embeddings = torch.stack(subgraph_embeddings)
                return entity_embeddings, subgraph_embeddings
            else:
                return entity_embeddings, torch.empty(0, self.out_dim, device=device)
        else:
            return node_embeddings['entity']

    def _forward_cpu(self, node_ids, return_subgraph):
        """CPU回退"""
        device = torch.device('cpu')
        ...
        # 通过异构图神经网络
        node_embeddings = self.hetgnn(self.graph, node_features)
...
def build_fb15k_graph(train_triples, num_entities, num_relations, device='cpu'):
    ...
    return g
def build_fb15k_graph_for_gpu(train_triples, num_entities, num_relations, device='cuda', max_edges=None):
    """为GPU构建异构图 - 支持大数据集"""
    # 收集所有边
    head_list = []
    tail_list = []
    ...
    if max_edges is None:
        # 使用所有边，但可以设置上限
        max_edges = min(500000, len(train_triples))  # 最多50万条边

    print(f"使用 {max_edges} 条边构建图 (总共 {len(train_triples)} 条)")
    sample_triples = train_triples[:max_edges]
    ...
    # 转换为张量
    head_tensor = torch.tensor(head_list, dtype=torch.long, device=device)
    tail_tensor = torch.tensor(tail_list, dtype=torch.long, device=device)
    rel_tensor = torch.tensor(rel_list, dtype=torch.long, device=device)
    ...
    print(f"图构建完成: {g.num_edges()} 条边, {g.num_nodes()} 个节点")
    return g
...
def train_fb15k_gpu(model, train_triples, num_epochs=50, lr=0.001,
                    device='cuda', use_contrastive=True, phase=1):
    """GPU优化训练函数 - 大数据集版本"""
    model = model.to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=lr, weight_decay=1e-5)

    # 使用部分数据进行训练（避免内存问题）
    train_sample_size = min(100000, len(train_triples))  # 使用10万条数据
    train_triples_sample = train_triples[:train_sample_size]
    print(f"训练使用 {train_sample_size} 条数据")

    # 转换为张量并移动到设备
    train_triples_tensor = torch.tensor(train_triples_sample, dtype=torch.long, device=device)
    ...
    for epoch in range(num_epochs):
        ...
        # 随机打乱数据
        indices = torch.randperm(len(train_triples_tensor), device=device)
        total_loss = 0.0
        num_batches = 0
...
def main():
    ...
    # 构建异构图
    # 构建异构图
    print('Building heterogeneous graph for GPU...')
    try:
        # 使用完整训练数据构建图（但限制边数）
        max_edges = 300000  # 30万条边，根据GPU内存调整
        g = build_fb15k_graph_for_gpu(
    ...
    # 创建模型 - 使用完整实体和关系数量
    print('Creating model for full dataset...')
    model = FB15K_XGradNet_GPU(
        g,
        num_entities,  # 完整实体数量
        num_relations,  # 完整关系数量
        feature_dim=128,  # 根据GPU内存调整
        hidden_dim=128,
        out_dim=64
    )
    model = model.to(device)
...
def main_gpu():
    ...
    # 3. 构建图（使用部分边避免内存问题）
    print('\n构建图...')
    max_edges = 200000  # 20万条边
    g = build_fb15k_graph_for_gpu(
        train_triples[:max_edges], num_entities, num_relations, device=device
    )
    ...
    # 第一阶段：基础训练
    print('第一阶段：基础训练')
    train_sample = train_triples[:50000]  # 5万条数据
    model, train_losses1 = train_fb15k_gpu(
        model, train_sample, num_epochs=15, lr=0.001,
        device=device, use_contrastive=False, phase=1
    )

    # 第二阶段：对比学习微调
    print('\n第二阶段：对比学习微调')
    model, train_losses2 = train_fb15k_gpu(
        model, train_triples[:20000], num_epochs=10, lr=0.0003,
        device=device, use_contrastive=True, phase=2
    )
    ...
    # 6. 评估模型
    print('\n评估模型...')
    test_sample = test_triples[:5000]  # 5千条测试数据
    accuracy = evaluate_fb15k(model, test_sample, device=device, batch_size=512)
    ...
    }, 'fb15k_igradnet_full.pth')

    print(f'\n训练完成!')
    print(f'模型保存到: fb15k_igradnet_full.pth')
...
if __name__ == '__main__':
    ...
    print("开始完整数据集训练...")
    model, accuracy, train_losses = main_gpu()
