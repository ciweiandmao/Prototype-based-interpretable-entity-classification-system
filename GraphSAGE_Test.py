...
def create_test_features_for_sage(valid_entities, model_data, entity_relations, entity_to_true_type):
    ...
    type_to_idx = model_data.get('type_to_idx', {})
    all_types = model_data.get('all_types', [])
    ...
    for entity in tqdm(valid_entities, desc="创建特征"):
        ...
        if saved_features is not None and entity in entity_to_idx:
            ...
            unique_neighbor_types = len(set(neighbor_types)) if neighbor_types else 0
            ...
            if neighbor_types:
                ...
                most_common_neighbor_type = type_to_idx.get(most_common_type, 0)
            ...
            # 组合所有特征
            if top_relations and all_types:
                feat_vector = np.concatenate([base_feat, rel_pattern_feat, neighbor_type_feat])
            elif top_relations:
                feat_vector = np.concatenate([base_feat, rel_pattern_feat])
            elif all_types:
                feat_vector = np.concatenate([base_feat, neighbor_type_feat])
            else:
                feat_vector = np.array(base_feat, dtype=np.float32)

            # 检查维度
            if len(feat_vector) != feature_dim:
                print(f"警告: 创建的特征维度 {len(feat_vector)} 与期望维度 {feature_dim} 不匹配")
