...
class MemoryEfficientHetGNN(nn.Module):
    def __init__(self, in_feats, hid_feats, out_feats, rel_names):
        ...
        self.bilstm = nn.LSTM(
            ...
            hidden_size=out_feats // 4,  # 减小隐藏层大小
            num_layers=1,
            batch_first=True,
            bidirectional=True
        )

        # 投影层，将BiLSTM输出映射回out_feats
        self.projection = nn.Linear(out_feats // 2, out_feats)
    ...
    def forward(self, graph, inputs, batch_nodes=None):
        ...
        if batch_nodes is None:
            ...
            if 'entity' in h:
                ...
                entity_feats = self.projection(entity_feats)
                h['entity'] = entity_feats
        else:
            # 批量模式 - 只处理batch_nodes及其邻居
            batch_nodes_tensor = torch.tensor(batch_nodes, device=graph.device)

            # 提取包含batch_nodes及其邻居的子图
            subgraph = dgl.sampling.sample_neighbors(
                graph, {'entity': batch_nodes_tensor}, fanout=20
            )

            # 获取原始图中的节点ID到子图节点ID的映射
            ...
            for ntype in inputs:
                if ntype in subgraph.ntypes:
                    # 子图中的节点ID是重新索引的，需要从原始图中获取特征
                    original_node_ids = subgraph_entity_nodes
            ...
            if 'entity' in h:
                ...
                # 找到batch_nodes在子图中的位置
                # 创建一个映射：原始节点ID -> 子图节点ID
                original_to_subgraph = {int(original_id): int(subgraph_id)
                                        for subgraph_id, original_id in enumerate(subgraph_entity_nodes)}

                batch_node_positions = []
                for node_id in batch_nodes:
                    if node_id in original_to_subgraph:
                        batch_node_positions.append(original_to_subgraph[node_id])

                if batch_node_positions:
                    batch_mask = torch.zeros(len(entity_feats), dtype=torch.bool, device=entity_feats.device)
                    batch_mask[batch_node_positions] = True

                    batch_feats = entity_feats[batch_mask]
                    batch_feats = batch_feats.unsqueeze(1)
                    batch_feats, _ = self.bilstm(batch_feats)
                    batch_feats = batch_feats.squeeze(1)
                    batch_feats = self.projection(batch_feats)

                    # 更新对应位置的节点特征
                    entity_feats[batch_mask] = batch_feats
...
class BatchSubgraphBuilder:
    ...
    def get_neighbors_batch(self, node_ids, fanout=20):
        """批量获取节点的邻居"""
        batch_neighbors = {}
        ...
        for node_id in node_ids:
            if node_id not in self.neighbor_cache:
                ...
                frontiers = dgl.sampling.sample_neighbors(
                    self.g, {'entity': torch.tensor([node_id], device=self.g.device)}, fanout=fanout
                )
                neighbors = frontiers.nodes('entity').cpu().numpy()
                self.neighbor_cache[node_id] = neighbors

            batch_neighbors[node_id] = self.neighbor_cache[node_id]

        return batch_neighbors
    def build_subgraph_embeddings_batch(self, node_ids, node_feats, embedding_weights=None, batch_size=32):
        """批量构建子图嵌入"""
        ...
        for i in range(0, len(node_ids), batch_size):
            ...
            for node_id in batch_nodes:
                ...
                if len(neighbors) > 1:
                    # 排除中心节点自身
                    other_neighbors = neighbors[neighbors != node_id]
                    if len(other_neighbors) > 0:
                        neighbor_feats = node_feats['entity'][other_neighbors]
                ...
                # 清理中间变量释放内存
                del neighbors, center_emb, relation_emb, components
...
class MemoryEfficientEntityGradNet(nn.Module):
    def __init__(self, hetero_graph, feature_dims, hidden_dim=64, out_dim=32, num_classes=9, category_dim=9):
        super().__init__()
        self.g = hetero_graph
        self.feature_dims = feature_dims
        self.hidden_dim = hidden_dim  # 减小隐藏层维度
        self.out_dim = out_dim  # 减小输出维度
        self.num_classes = num_classes
    ...
    def classify_batch(self, node_embeddings, node_ids, batch_size=32, update_prototypes=False, labels=None):
        """批量分类"""
        subgraph_embeddings = []
        components_list = []
        ...
        for i in range(0, len(node_ids), batch_size):
            ...
            # 清理内存
            del batch_embeddings, batch_components
            if torch.cuda.is_available():
                torch.cuda.empty_cache()
    ...
    def analyze_prediction_batch(self, node_ids, node_embeddings, true_labels=None, batch_size=32):
        ...
        for i in range(0, len(node_ids), batch_size):
            ...
            for j, node_id in enumerate(batch_nodes):
                ...
                if components:
                    structure_contributions = {
                        'center': torch.norm(components['center']).item() if 'center' in components else 0.0,
                        'relations': torch.norm(components['relations']).item() if 'relations' in components else 0.0
                    }
                ...
                if similarities is not None:
                    similarity_dict = {self.class_names[i]: similarities[i].item() for i in
                                       range(min(len(self.class_names), similarities.size(0)))}
                ...
                result = {
                    ...
                    'prediction': {
                        ...
                        'probability': probs[j, pred_classes[j]].item() if j < pred_classes.size(0) else None
                    },
...
def train_batch(model, node_features, train_idx, val_idx, labels, num_epochs=50, lr=0.001,
    ...
    for epoch in range(num_epochs):
        ...
        # 分批训练
        for i in range(0, len(train_idx_array), train_batch_size):
            batch_indices = train_idx_array[i:i + train_batch_size]
        ...
        if (epoch + 1) % 5 == 0 or epoch == num_epochs - 1:
            ...
            with torch.no_grad():
                ...
                # 分批验证
                for i in range(0, len(val_idx_array), eval_batch_size):
                    batch_indices = val_idx_array[i:i + eval_batch_size]
...
def evaluate_batch(model, node_features, test_idx, true_labels, class_names, device=None, batch_size=128):
    ...
    if sample_size > 0:
        ...
        explanations = explainer.analyze_prediction_batch(
            ...
            true_labels[sample_indices].cpu().numpy() if len(true_labels) > 0 else None,
            batch_size=batch_size
        )
    else:
        explanations = []
